...
#load("gse.rda")
@
Next we coerce the raw data from GEO into an ExpressionSet which is the most recent class for holding gene expression data within Bioconductor. See the GEOquery vignette for more information. 
...
rownames(data.matrix) <- as.character(probesets)
colnames(data.matrix) <- names(GSMList(gse))
pdata <- data.frame(samples = names(GSMList(gse)))

subtype <- gsub("\\.","_",unlist(lapply(GSMList(gse),function(x) gsub("subtype: ","", x@header$characteristics_ch1))))
pheno <- new("AnnotatedDataFrame", data=data.frame(subtype), varMetadata=data.frame(labelDescription=c("Renal Cell Carcinoma Subtype (NO=normal tissue)")))
...
\begin{center}
<<fig=TRUE, echo=TRUE>>=
smcPlot(pg, factor(subtype), col=.rwb, scale=c(-15, 15), margins=c(1, 1, 6, 9), show.grid=T, r.cex=.75)
@
\end{center}
...
topTable(fit, n=10)[,c("ID","logFC","t","adj.P.Val")]
@

Now that we have figured out the significantly different gene sets, we can grab the "ID" column from "topTable" and insert it into our plotting function.

\begin{center}
<<fig=TRUE, echo=TRUE>>=
smcPlot(pg[topTable(fit, n=10)$ID,], factor(subtype,levels=c("P1","P2B")), col=.rwb, scale=c(-15, 15), margins=c(1, 1, 6, 19), show.grid=T, r.cex=.75)
@
\end{center}
...
\begin{center}
<<fig=TRUE, echo=TRUE>>=
smcPlot(pg[topTable(fit,n=30,resort.by="logFC")$ID,], factor(subtype,levels=c("P1","P2B")), col=.rwb, scale=c(-15, 15), margins=c(1, 1, 6, 19), show.grid=T, r.cex=.75)
@
\end{center}
