...
library(GEOquery)
library(GSEABase)
gse <- getGEO("GSE7023",GSEMatrix=T)
#load("gse.rda")
@
...
subtype <- gsub("\\.", "_",gsub("subtype: ", "", gse[[1]]@phenoData@data$"characteristics_ch1"))
pheno <- new("AnnotatedDataFrame", data = data.frame(subtype), varMetadata = data.frame(labelDescription="subtype"))
rownames(pheno@data) <- colnames(gse[[1]]@assayData$exprs)
eset <- new("ExpressionSet", exprs = gse[[1]]@assayData$exprs, phenoData = pheno)
