...
@
	...
	As you can see, "gene a" and "gene b" were used in this example. Unfortunately genes are usually not annotated with such simplicity. Many different identifiers can be encountered when searching for expression data sets or gene lists. Not only are different identifiers used between different databases, sometimes multiple probes on the array can map to a single gene. This makes converting a large dataset complicated. To help with this problem, we have included \Rfunction{aggregateExprs}. This function will convert the row names of an expression matrix, geneNames of an eSet, or reporterNames of an exprSet to your desired format using the associated annotation environment. A function can also be included to collapse data from multiple probes to a single summary value.  By default \Rfunction{aggregateExprs} uses the mean of the duplicated probe values, but other summary functions, such as the "absolute maximum value" found in the \Rfunction{absMax} function (found in the package \Rpackage{reb}) are also worth considering.

\section{Concepts from ".gmt" files}
	A previous published method of gene expression termed "Gene Set Enrichment Analysis" \url{http://www.broad.mit.edu/gsea/} also used lists of related genes and to hold their gene lists they use a file format called ".gmt" and we have found it to be a simple and useful way to store our lists as well. To facilitate using this format in conjunction with our "smc" class, we have included the functions \Rfunction{readGmt} and \Rfunction{writeGmt}. An example of how to read in a ".gmt" file is shown below. The file provided contains some example molecular concepts. Genes induced and inhibited by Ras and Myc, as well as all genes on chromosome arms 5p and 5q have been included. 
...
<<echo=TRUE, print=FALSE>>=
	...
	data(nbEset)
	pg <- PGSEA(nbEset@exprs,cl=sample,ref=1:5)
@ 
...
\section{Using Gene Ontologies as concepts}
	...
	Next, for our list of concepts we will use the function \Rfunction{go2smc}. This function will return a very large list of concepts, so we will just use a few of them for illustration. 

<<echo=TRUE, print=FALSE>>=
	mcs <- go2smc()[1:10]
	pg <- PGSEA(nbEset@exprs,cl=mcs,ref=1:5)
...
\section{Concepts compiled at VAI}
	...
	Lastly, we have included a list of concepts that we have manually compiled from various sources. We have found these concepts to very informative in our analysis. 


<<echo=TRUE, print=FALSE>>=
	data(VAImcs)
	pg <- PGSEA(nbEset@exprs,cl=VAImcs,ref=1:5)
